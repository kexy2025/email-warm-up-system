...
def validate_smtp_connection(provider, email, username, password, smtp_host=None, smtp_port=587, use_tls=True):
    try:
        ...
        if provider in ['custom_smtp'] and not smtp_host:
...
def campaigns():
    if request.method == 'GET':
        ...
        try:
            ...
            if provider not in SMTP_PROVIDERS and provider not in ['custom_smtp']:
    ...
    return jsonify({
        'providers': {
            key: {
                ...
                'requires_custom_host': key in ['custom_smtp']
            }
